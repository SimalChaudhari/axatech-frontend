 ...
 * @param {string} [variant='primary'] - 'primary' | 'secondary' | 'outline' | 'danger' | 'ghost' | 'inverse'
...
export default function Button({
  ...
  const baseClass = `py-3 px-4 font-semibold rounded-[10px] cursor-pointer transition-colors ${
    variant === 'secondary'
      ...
      : variant === 'outline'
        ...
        : variant === 'danger'
          ? 'text-white bg-red-600 border-0 hover:bg-red-700 dark:bg-red-600 dark:hover:bg-red-700'
          : variant === 'ghost'
  ...
  const fullClass = `${widthClass}${baseClass} ${isButton ? 'border-0 disabled:opacity-60 disabled:cursor-not-allowed' : 'inline-flex items-center justify-center'} ${className}`.trim();
  ...
  return (
    ...
    >
      {loading && isButton ? loadingLabel : children}
    </ResolvedComponent>
  );
}
